...
4. Create also a `form.json` file that defines a standard form that will be used for creating and editing records in the table, referring to the fields in `fields.json` in a layout that contains a hidden section and two columns. The schema is `schemas/form.schema.json`.
